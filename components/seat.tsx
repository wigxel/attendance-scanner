...
export default function SeatComponent({
  ...
  const isSelected = seat.some((item) =>
    item.seatAllocation.includes(seatId.seatOption),
  );
  ...
  const getSeatOptions = () => {
    return seat
      .filter(
        (s) =>
          s.seatAllocation.startsWith("t") || s.seatAllocation.startsWith("H"),
      )
      .map((s) => s.seatAllocation);
  };
  ...
  const handleSeatSelection = () => {
    ...
    const currentSeat = {
      seatAllocation: seatId.seatOption,
      label: seatId.name,
      seatStatus: "seatReserved",
    };

    const isAlreadySelected = seat.some(
      (s) =>
        s.seatAllocation === currentSeat.seatAllocation &&
        s.label === currentSeat.label,
    );
  ...
  return (
    ...
    >
      <span
        className={`${textAlignment} text-xs ${seat.some((s) => s.seatAllocation === seatId.seatOption) && "text-white"}`}
      >
        {seatId.name}
        {/* seat bar */}
