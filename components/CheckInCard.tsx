...
function QRCode() {
  ...
  const qr_hash = useQueryHash(
    [device_meta.data?.visitorId, device_meta?.data]
  )

  const isLoading = device_meta.isLoading || profile.isLoading || qr_hash.isLoading
  ...
  return (
    <m.div
      ...
      {isLoading ? <div className="absolute inset-0 bg-white/[0.3] z-20 backdrop-blur-lg flex justify-center items-center">
        <div className='bg-white rounded-full p-4'>
          <LucideLoader size={'2rem'} strokeWidth={1} className='animate animate-spin' />
        </div>
      </div>
        : null}
      ...
      <div ref={ref} className={"w-full aspect-square p-2 relative"}>
        {size === null ? null :
          <QRCodeSVG
            size={size}
            fgColor={"#111"}
            value={qr_hash.data ? new URL(`/scan/${qr_hash.data}`, APP_URL).toString() : "nice-try-nothing-to-see-here"}
