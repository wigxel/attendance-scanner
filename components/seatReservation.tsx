...
import {
  TABLE_LAYOUT_SECTION_1,
  TABLE_LAYOUT_SECTION_2,
  TABLE_LAYOUT_SECTION_3,
  TableCfg,
} from "@/lib/tableData"; //data for refactored table;
...
export default function SeatReservationComponent({
  ...
  const handleNextStep = async () => {
    if (seat.length === 0) {
      ToastComponentProps({
        type: "error",
        message: "Kindly select a table and a seat(s)",
      });
      return false;
    } else if (seat.length !== numberOfSeats) {
      ToastComponentProps({
        type: "error",
        message: "Seat selection should match number of seats.",
      });
      return false;
    } else {
  ...
  return (
    <section className="w-full h-fit flex justify-center items-start p-4 xl:p-0 mt-10 xl:mt-0">
      ...
      <div
        className={
          "w-[335px] sm:w-[335px] h-fit flex flex-col justify-center items-center"
        }
      >
        {/* reservation */}
