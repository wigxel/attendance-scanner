...
export const parseEither = <A, ErrorType>(data: unknown): Either.Either<A, ErrorType> => {
  if (typeof data !== "object" || data === null) throw new Error("Invalid Either JSON");
  const objectData = data as Record<string, unknown>;
  if ("_tag" in objectData) {
    if (objectData._tag === "Right") return Either.right(objectData.right as A);
    if (objectData._tag === "Left") return Either.left(objectData.left as ErrorType);
    if (objectData._tag === "Ok") return Either.right((objectData as { value: A }).value);
    if (objectData._tag === "Err") return Either.left((objectData as { error: ErrorType }).error);
  }
