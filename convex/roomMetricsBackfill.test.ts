...
describe("roomMetrics backfill RED", () => {
  it("startRoomMetricsBackfill exists and rejects invalid date", async () => {
    ...
    await expect(
      t.mutation(internal.roomMetrics.startRoomMetricsBackfill, {
        from: "bad-date",
      } as any),
    ).rejects.toThrow();
  });
  ...
  it("startRoomMetricsBackfill sets lock", async () => {
    ...
    const res: any = await t.mutation(
      internal.roomMetrics.startRoomMetricsBackfill,
      { from, to, target: "10m" },
    );
    ...
    const lock = await t.run(async (ctx) => {
      return await ctx.db
        .query("config")
        .withIndex("by_key", (q) => q.eq("key", "roomMetrics_backfill_lock"))
        .unique();
    });
  ...
  it("backfillRoomMetrics10m aggregates raw 7d window into 10m buckets", async () => {
    ...
    await t.mutation(api.roomMetrics.store, {
      temperature: 20,
      humidity: 40,
      pressure: 1000,
      timestamp: base + 1000,
    ...
    await t.mutation(internal.roomMetrics.backfillRoomMetrics10m, {
      cursor: base - 1,
      to: base + DAY,
      target: "10m",
    });
    const buckets = await t.query(api.roomMetrics.history, {
      roomId: "all",
      limit: 10,
    });
  ...
  it("backfillRoomMetricsDaily handles >3 months (90 days)", async () => {
    ...
    await t.mutation(internal.roomMetrics.backfillRoomMetricsDaily, {
      cursor: -1,
      to: dayStart(90),
    });
    const daily = await t.query(api.roomMetrics.getDailyCalendar, {
      roomId: "all",
      days: 100,
    });
  ...
  it("surgical destroy deletes only range", async () => {
    ...
    await t.mutation(internal.roomMetrics.startRoomMetricsBackfill, {
      from: toDateStr(d10),
      to: toDateStr(d10),
      target: "10m",
      destroy: true,
    ...
    expect(
      buckets.some(
        (b: any) =>
          b.bucketStart === Math.floor((d5 + 1000) / TEN_MIN) * TEN_MIN,
      ),
    ).toBe(true);
  });
  ...
  it("live smoothAggregations bails when backfill lock held", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("config", {
        key: "roomMetrics_backfill_lock",
        value: "true",
      });
      await ctx.db.insert("config", {
        key: "roomMetrics_backfill_heartbeat",
  ...
  it("cancelRoomMetricsBackfill clears lock", async () => {
    const t = convexTest(schema, modules);
    await t.mutation(internal.roomMetrics.startRoomMetricsBackfill, {
      from: toDateStr(dayStart(1)),
      to: toDateStr(dayStart(2)),
      target: "10m",
    });
    await t.mutation(internal.roomMetrics.cancelRoomMetricsBackfill, {});
    const lock = await t.run(
      async (ctx) =>
        await ctx.db
          .query("config")
          .withIndex("by_key", (q) => q.eq("key", "roomMetrics_backfill_lock"))
