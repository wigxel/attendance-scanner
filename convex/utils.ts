...
export const fixDuplicates = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const email of duplicateEmails) {
      ...
      deletedRecordIds = [...deletedRecordIds, ...newly_deleted_emails];
    }
  ...
  async handler(ctx, args) {
    ...
    const entries = await Promise.all(
      profilesWithThisEmail.map(async (p) => {
        ...
        return [
          p,
          await ctx.db.get(p.id as Id<"users">).catch(() => null),
        ] as const;
      }),
    );
    ...
    if (profilesWithThisEmail.length > 1) {
      ...
      const profilesToDelete = profilesWithThisEmail.filter(
        (_, index) => index !== pass_index,
  ...
  async handler(ctx, args) {
    ...
    const maps = new Map([
      [
        "daily_register",
        [
          { search_index: "admitted_by", column: "admitted_by" },
          { search_index: "user", column: "userId" },
        ],
      ],
    ...
    async function update<T extends TableNames>(
      ...
      const lazy_process = matches.map((match) => {
        return () => {
          console.info(
            `Updating ${table}:${column.toString()} record with ID: ${match._id}: ${old_id} to ${new_id} `,
          );
    ...
    (await Promise.all(ops.flat())).flat().map((fn) => fn());
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    for (const entry of all.slice(1)) {
      ...
      if (match?.email) {
        if (dryRun) {
          console.log(
            `Dry run: Would update ${entry.id} with email ${match?.email}`,
          );
        } else {
